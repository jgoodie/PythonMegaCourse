...
def search_cmd():
    lbox.delete(0, END)
    for row in bookstore_backend.search_e(title_text.get(), author_text.get(), year_text.get(), isbn_text.get()):
        lbox.insert(END, row)

def addentry_cmd():
    bookstore_backend.add_e(title_text.get(), author_text.get(), year_text.get(), isbn_text.get())    
...
def delete_cmd():
    bookstore_backend.delete_e(selected_tuple[0])

def update_cmd():
    bookstore_backend.update_e(selected_tuple[0],title_text.get(),author_text.get(),year_text.get(),isbn_text.get())
